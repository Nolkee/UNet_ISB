...
- 已完成：Stage-1 验证阶段自动保存少量 `input / prediction / target` 三联图
- 已完成：Stage-1 训练结束后自动评估并输出 PSNR / SSIM / LPIPS
...
3. 我没有在本机做训练实跑
   代码已做静态审计和逻辑修补，但最终正确性仍然需要你在远程 Ubuntu 服务器上用真实数据跑通一次前向和训练。

## 仓库结构

- [train_stage1_restoration.py](train_stage1_restoration.py)
  第一阶段训练入口
- [stages/stage1/train.sh](stages/stage1/train.sh)
  第一阶段标准启动脚本
- [stages/stage1/verify_dataset.py](stages/stage1/verify_dataset.py)
  第一阶段数据集验证脚本
- [stages/stage2/train.sh](stages/stage2/train.sh)
  第二阶段占位脚本
- [stages/stage3/train.sh](stages/stage3/train.sh)
  第三阶段占位脚本
...
- [run_train.sh](run_train.sh)
  向后兼容的 Stage-1 启动包装脚本
- [configs/stage1_config.sh](configs/stage1_config.sh)
  向后兼容的 Stage-1 配置包装脚本
...
Stage-1 在有验证集时会默认每个 epoch 保存少量固定样本的三联图，并在训练结束后自动对验证集做一次最终评估：
...
- `--val-save-count 4`
- `--val-save-subdir val_triplets`

最终评估会优先读取 `best_stage1.pth`；如果该文件不存在，则退回到最后一个 `checkpoint_epoch_*.pth`，再不行才使用内存中的最终权重。
...
checkpoints_stage1/
  ...
  val_triplets/
    epoch_001/
      0000_sample_input.png
      0000_sample_pred.png
      0000_sample_target.png
```

`final_eval_metrics.json` 会记录最终评估所使用的 checkpoint 名称以及 `sample_count / psnr / ssim / lpips`。这个子集按验证集顺序固定选择，用于跨 epoch 做稳定的可视化对比；如果不想保存三联图，可传 `--val-save-count 0`。
