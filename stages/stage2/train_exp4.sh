#!/bin/bash
set -euo pipefail
# Stage-2: PatchGAN 调参 Exp-4
# 在 Exp-1 基础上增强 patch-NCE，偏向感知质量
...
SAVE_DIR="${SAVE_DIR:-$REPO_ROOT/checkpoints_stage2_exp4_20260411}"
...
CMD=(
  ...
  --patch-nce-weight 0.20
  --bro-weight 0.03
  --irc-weight 0.0

  # Adversarial (Exp-4 = Exp-1 + stronger PatchNCE)
  --adv-weight 0.8
  --warmup-epochs 8
...
echo "=========================================="
echo " Stage-2: PatchGAN Tuning (Exp-4)"
