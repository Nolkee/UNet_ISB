"""Stage-2 training: SB-EQ U-Net generator + PatchGAN discriminator.

Loads Stage-1 best checkpoint and continues training with adversarial pressure.
Content losses are delegated to the existing Stage1RestorationLoss; the adversarial
component is ramped in over a configurable warmup period.

Usage:
    python train_stage2_restoration.py \
        --load-stage1 checkpoints_stage1_detail_mild/best_stage1.pth \
...
from utils.restoration_losses import Stage1RestorationLoss, Stage2RestorationLoss, LSGANLoss
from utils.restoration_metrics import RestorationMetricTracker, ensure_restoration_metric_dependencies

# Reuse shared utilities from Stage-1 training
...
def create_argparser():
    parser = argparse.ArgumentParser(description='Train stage-2 SB-EQ U-Net with PatchGAN adversarial loss')
    ...
    parser.add_argument('--save-dir', type=str, default='checkpoints_stage2', help='Checkpoint output directory')
    parser.add_argument('--device', type=str, default='auto', help='Training device')

    # Stage-1 checkpoint (REQUIRED)
    parser.add_argument('--load-stage1', type=str, required=True,
                        help='Path to Stage-1 best checkpoint (required)')

    # Resume Stage-2 training
    parser.add_argument('--resume', type=str, default='',
                        help='Resume Stage-2 training from a Stage-2 checkpoint')

    # Training hyperparameters
    parser.add_argument('--epochs', '-e', type=int, default=50)
    parser.add_argument('--batch-size', '-b', dest='batch_size', type=int, default=4)
    parser.add_argument('--g-lr', type=float, default=1e-4, help='Generator learning rate')
    parser.add_argument('--d-lr', type=float, default=2e-4, help='Discriminator learning rate')
    ...
    parser.add_argument('--ndf', type=int, default=64, help='Discriminator base filter count')
    parser.add_argument('--n-layers', type=int, default=3, help='Discriminator depth (3 → 70x70 RF)')
    ...
    parser.add_argument('--bro-weight', type=float, default=0.03)
    parser.add_argument('--irc-weight', type=float, default=0.0)
    ...
    # Adversarial loss
    parser.add_argument('--adv-weight', type=float, default=1.0, help='Adversarial loss weight (after warmup)')
    parser.add_argument('--warmup-epochs', type=int, default=5, help='Epochs to linearly ramp adversarial weight')
...
def compute_d_accuracy(real_scores: torch.Tensor, fake_scores: torch.Tensor) -> tuple[float, float]:
    """Compute discriminator classification accuracy for monitoring."""
...
def evaluate(
    generator, stage2_criterion, loader, device, amp,
    epoch: int, val_save_count: int, val_save_dir: Path | None,
):
    """Validation loop — content losses only (no adversarial)."""
    generator.eval()
    stage2_criterion.eval()
    history = []
    ...
    with torch.no_grad():
        for step, batch in enumerate(loader, start=1):
            ...
            # Use stage1 content criterion for validation (no adversarial component)
            _, content_metrics = stage2_criterion.stage1_criterion(outputs, batch)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        with tqdm(total=len(train_loader.dataset), desc=f'Epoch {epoch}/{args.epochs}', unit='img') as pbar:
            for step, batch in enumerate(train_loader, start=1):
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=args.amp):
                    ...
                    g_loss, metrics = stage2_criterion(outputs, batch, fake_scores_g, epoch)

                raise_if_non_finite(g_loss, 'g_loss', epoch, step)
                g_scaler.scale(g_loss).backward()
                g_scaler.unscale_(g_optimizer)
                torch.nn.utils.clip_grad_norm_(
                    list(generator.parameters()) + list(stage2_criterion.parameters()), args.grad_clip
                )
        ...
        if val_loader is not None:
            val_metrics = evaluate(
                generator, stage2_criterion, val_loader, device, args.amp,
        ...
        checkpoint = {
            ...
            'criterion_state_dict': stage2_criterion.state_dict(),
        ...
        if val_metrics and val_metrics.get('loss_total', float('inf')) < best_val:
            ...
            torch.save(checkpoint, save_dir / 'best_stage2.pth')
...
def main():
    ...
    # ---- Build generator & load Stage-1 weights ----
    generator = SBEQUNet(
    ...
    stage1_ckpt = torch.load(args.load_stage1, map_location=device)
    generator.load_state_dict(stage1_ckpt['model_state_dict'], strict=False)
    logging.info('Loaded Stage-1 generator weights from %s', args.load_stage1)
    ...
    logging.info('Built PatchGAN discriminator: ndf=%d, n_layers=%d', args.ndf, args.n_layers)
    ...
    # Restore Stage-1 criterion learned parameters (PatchNCE projectors, etc.)
    if 'criterion_state_dict' in stage1_ckpt:
        stage1_criterion.load_state_dict(stage1_ckpt['criterion_state_dict'])
        logging.info('Restored Stage-1 criterion state (PatchNCE projectors, IRC heads)')
    ...
    # ---- Optimizers (TTUR: separate G/D learning rates) ----
    g_optimizer = optim.AdamW(
        list(generator.parameters()) + list(stage2_criterion.parameters()),
        lr=args.g_lr,
        betas=(0.5, 0.999),
    ...
    # ---- Resume Stage-2 checkpoint if provided ----
    start_epoch = 1
    best_val = float('inf')
    if args.resume:
        ...
        if 'criterion_state_dict' in resume_ckpt:
            stage2_criterion.load_state_dict(resume_ckpt['criterion_state_dict'])
        ...
        logging.info('Resumed Stage-2 training from %s (epoch %d)', args.resume, start_epoch)
    ...
    if args.irc_weight > 0 and not train_set.has_valid_labels:
        raise ValueError(
            'IRC loss requires degradation labels, but no valid labels found. '
            'Provide degradation_label in manifest or set --irc-weight 0.'
        )
    ...
    if start_epoch > 1:
        logging.info('Resuming stage-2 training from epoch %d', start_epoch)
